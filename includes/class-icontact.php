...
class iContact {
	...
	public function make_request( $action = null, $options = array(), $method = 'GET', $return_key = null ) {
		...
		if ( is_wp_error( $response ) ) {
			...
			if ( isset( $response['errors'] ) ) {
				return new WP_Error( wp_remote_retrieve_response_code( $response ), $response['errors'][0] );
			}

			if ( isset( $response['warnings'] ) ) {
				return new WP_Error( wp_remote_retrieve_response_code( $response ), $response['warnings'][0] );
			}
	...
	public function set_account_id() {
		...
		if ( empty( $this->account_id ) ) {
			...
			if ( isset( $accounts['errors'] ) ) {
				return new WP_Error( 'iContact_accounts_error', $accounts['errors'][0] );
			}
			...
			if ( $account['enabled'] == 1 ) {
				...
				return new WP_Error( 'iContact_account_error', 'Your account has been disabled.' );
	...
	public function add_contact_to_list( $contact_id, $list_id, $status = 'normal' ) {
		...
		return $this->make_request( $this->get_url_base() . 'subscriptions', array( $subscription ), 'POST', 'subscriptions' );
	...
	public function get_client_folders() {
		...
		if ( isset( $clients['errors'] ) ) {
			return new WP_Error( 'iContact_client_folders_error', 'No client folders were found for this account.' );
		}
